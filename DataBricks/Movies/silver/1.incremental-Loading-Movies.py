...
watermark=spark.read.table("db_projects.dev.wateremark").filter("process_name='Ingest movies'").select("watermark_time").collect()[0][0]
...
def getSchema():
    schema="""movieId integer, title string, genres string
            """
    return schema
...
try:
    def process():
        ...
        moviesDF = readFile(file_schema)
        moviesDF.dropDuplicates().write.mode("append").saveAsTable("db_projects.dev.movies_sv")
        spark.sql("""update db_projects.dev.wateremark
                    set watermark_time=current_timestamp()
                    where process_name='Ingest movies'""")
        print("Done")
        audit(moviesDF,'db_projects.dev.movies_sv')
    process()
